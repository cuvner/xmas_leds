...
# Best-effort WAV playback on the Cosmic Unicorn speaker
try:
...
def ensure_music():
    """Start or restart the Christmas WAV loop when possible."""
    ...
    try:
        if not sound.is_playing():
            # Loop the track so it accompanies the slideshow continuously.
            sound.play_wav("christmas.wav", True)
    except Exception:
        # If playback fails once, avoid spamming errors.
        return
